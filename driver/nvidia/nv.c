 ...
 */
#if defined(NV_PCI_CLASS_MULTIMEDIA_HD_AUDIO_PRESENT)
    nv_linux_state_t *nvl = NV_GET_NVL_FROM_NV_STATE(nv);
    struct device *dev = nvl->dev;
...
#if defined(NV_SEQ_READ_ITER_PRESENT)
    {
        ...
        if (bEnter)
        {
            pm_runtime_put_noidle(&codec->core.dev);
        }
        else
        {
...
#else
    /*
     * Ideally, we need to honor bEnter. If it is true, then send the
     * request for suspending the HDA controller and if is false, then send the
     * request for waking up the HDA controller. The following workaround will
     * wake up the HDA controller for few seconds and then bring it back to
     * suspended state. We are using this workaround for both the cases
     * (bEnter is true or false). It will help in keeping HDA controller active
     * for the duration, when GPU is processing HDA controller commands
     * (like writing ELD data).
     */
    {
        int codec_addr;
