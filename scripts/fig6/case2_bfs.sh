...
run_benchmark() {
    ...
    docker logs huvm_harvestee_1 > ${output_path}/mobilenet_raw.txt
    docker kill huvm_harvestee_1
    docker logs huvm_harvestee_2 > ${output_path}/resnet_raw.txt
    docker kill huvm_harvestee_2
    output=` grep -o 'Time[^;]*Data' ${output_path}/mobilenet_raw.txt | tail -n 1` 
    output=`echo "${output:14:-6}"` 
    add_col_to_csv "${output_path}/mobilenet.csv" "${output_header}" "${output}"
